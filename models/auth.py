...
class DiscordBot(ABC):
    ...
    async def bot_request(
        self,
        route: str,
        method: Literal["GET", "POST", "PATCH", "PUT"] = "GET",
        **kwargs,
    ):
        if self.client_session is None:
            raise Exception("Client Session not initialized")
    ...
    async def fetch_members(
    ...
    ) -> Union[Optional[Member], List[Member]]:
        ...
        if not self._members.get("objects") or (
            ...
            members = [
                Member(**m)
                for m in await self.bot_request(
        ...
        if member_id:
            return next(
                (
                    m
                    for m in self._members["objects"]
    ...
    async def fetch_roles(
        ...
        if not self._roles["objects"] or (
            ...
            roles = [
                Role(**r)
                for r in await self.bot_request(f"/guilds/{DISCORD_GUILD_ID}/roles")
            ]
        ...
        if role_id:
            ...
            return next(
                (r for r in self._roles["objects"] if r.name == kwargs.get("name")),
                None,
            )
    ...
    def encode_tokens_to_jwt(self, access_token: str, refresh_token: str) -> str:
        ...
        payload = {
            ...
            + timedelta(days=7),  # JWT expires in 7 days
        }
