    ...
    if (type === "filter") {
        ...
        return properties
            .map((property) => {
                ...
                const matchingProperty: Property = propertiesData.find(
                    (prop: Property) => {
                        return (
                            prop.name.toLowerCase() === clean_prop.toLowerCase() &&
                ...
                if (matchingProperty) {
                    return `<span class="info-link" data-name="${escapeHtml(
                        matchingProperty.name
                    )}" data-text="${escapeHtml(
                        matchingProperty.text
                    )}">${escapeHtml(property)}</span>`;
                } else {
...
$(document).on('click', "#power-table tbody tr", function() {
    ...
    if (isDragging) return // Prevent click action if user was dragging
...
if ($("#equipment-table").length){
    const params = new URLSearchParams(window.location.search);    const tableName = "#equipment-table";
...
if ($("#item-table").length){
    const params = new URLSearchParams(window.location.search);    const tableName = "#item-table";
...
$(document).on('click', '#feat-table .edit-button', function(){
    ...
    const feat: Feat = table.rows().data().toArray().find((row: Feat) => row.id == featId);
    
    if (!feat) ToastError("Power not found")

    defaultFeatModal(feat)
})

$(document).on('click', '#new-feat-btn', function(){
    let feat: Feat = fetchFeatInputs()
...
$(document).on('click', "#maneuver-table tbody tr", function(){
    ...
    const row = table.row(this)
    const maneuver = row.data() as Maneuver
    let stop = false
...
$(document).on('click', '#maneuver-table .edit-button', function(){
    ...
    const maneuver: Maneuver = table.rows().data().toArray().find((row: Maneuver) => row.id == manId);
    
    if (!maneuver) ToastError("Maneuver not found")
...
$(document).on('click', "#customization-table tbody tr", function(){
    ...
    const row = table.row(this)
    const customization = row.data() as Customization
    let stop = false
...
$(document).on('click', '#customization-next', function(){
    ...
    let customization: Customization = fetchCustomizationInputs()
...
$(document).on('click', '#customization-table .edit-button', function(){
    ...
    const customization: Customization = table.rows().data().toArray().find((row: Customization) => row.id == objId);
    
    if (!customization) ToastError("Customization not found")
