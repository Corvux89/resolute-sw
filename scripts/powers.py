...
data = request.json()
output = [
    ['name', 'pre-requisite', 'type', 'casttime', 'range', 'source', 'description', 'concentration', 'alignment', 'level']
    ]

for power in data:
    line = [power.get('name'),
            power.get('prerequisite'),
            2 if power.get('powerType') == 'Tech' else 1 if power.get('powerType') == 'Force' else None,
